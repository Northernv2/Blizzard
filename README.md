<div align="center">
  <img src="https://cdn.discordapp.com/attachments/907083009688236043/907255613954850816/IMG_5054.png" align="center">
  <br>
  <strong><i>A feature-rich all-purpose bot for Discord.</i></strong>
  <br>
  <br>
  
  <a href="#">
    <img src="https://img.shields.io/badge/Latest%20Version-v0.2-7289da?style=for-the-badge&logo=data:image/gif;base64,">
  </a>

  
  <a href="https://www.dsc.gg/blizzardbot">
    <img src="https://img.shields.io/discord/907083009293963294.svg?label=Discord&logo=Discord&colorB=7289da&style=for-the-badge" alt="Support">
  </a>
  
  <a href="https://www.discord.js.org/downloads/">
    <img src="https://img.shields.io/badge/Made%20With-DiscordJS%2012.5.3-blue.svg?style=for-the-badge&logo=DiscordJS" alt="Made with DiscordJS 12.5.3">
  </a>
  
  <!-- TABLE OF CONTENTS -->
<details>
  <summary>Table of Contents</summary>
  <ol>
    <li>
      <a href="#what-is-blizzard">What is Blizzard?</a>
      <ul>
        <li><a href="#how-does-it-work">How does it work?</a></li>
      </ul>
    </li>
    <li>
      <a href="#features">Features</a>
      <ul>
        <li><a href="#to-do-list">To-do list</a></li>
        <li><a href="#installation">Installation</a></li>
      </ul>
    </li>
    <li><a href="#coming soon">Coming Soon!</a></li>
    <li><a href="#roadmap">Coming Soon!</a></li>
    <li><a href="#contributing">Coming Soon!</a></li>
    <li><a href="#license">Coming Soon!</a></li>
    <li><a href="#contact">Coming Soon!</a></li>
    <li><a href="#acknowledgments">Coming Soon!</a></li>
  </ol>
</details>
  
  
  ## What is Blizzard?

Blizzard is a multi-functional discord bot with over 100+ commands, and will have even more when development grows.

This bot is free for everyone and always will be. If you like this project and would like to show your appreciation, you can support us on [Discord](https://www.dsc.gg/blizzardbot), cool benefits included! 

## How does it work?

Well, there are 100+ commands so I can't explain them all, so you'll have to see for yourself!

## Features

* **Highly Customisable:**
  * Bot activity, prefix, category, log channel, etc.
  * Fun section
  * Warn system (warns and sends a DM to the user)
  * Many Moderation Commands
  * Anti-Bot Raid
  * Many many more!



This list is will be ever-growing if we get support and motivation. See a full list of documented commands by using the `$help` command.
  
  
  ## To-Do List

* **Things we want to add:**
  * Modmail System
  * Giveaway system
  * Ticket System
  * Fully automatic dashboard 
  * 300+ Commands
  * Many many more!

## Credits
  
  This bot was coded by [NorthernV2](https://www.github/NorthernV2)
